...
**Code snapshot (as of this doc):** `player.gd` uses instant horizontal speed (`velocity.x = move_x * SPEED`) and floor-only jump (`is_on_floor()`). Dig is an immediate tile erase via `terrain.dig_in_direction`. Feedback today is mostly HUD/text (`notice_message`, `frontier_notice`). No audio, particles, screen shake, freeze frames, coyote/buffer, or squash-stretch yet.
...
**Krater:** Good fit once jump/land moments exist. Player currently has 8-dir **idle** frames only — squash on land still works with a single frame. Dig strike could use a tiny forward squash toward aim, but keep Cap digs *quieter* visually than Pit digs (secrecy vs public danger).
...
| Cap dig (especially noticed) | Prefer *less* camera drama — secrecy; grit + quiet SFX over shake |
...
**Krater:** Use sparingly. Candidates: first Record found, Standing drop from exposed lie, Cap catch — *maybe* a single short hitch. Dig-every-tile freeze will make mining feel sticky and break the venture→return loop rhythm.

**Does not apply / careful:** No freeze on routine digs, siphon button clicks, or NPC dialogue open. Never stack freezes with long harvest timers in a way that desyncs the Harvest clock feel.
...
**Krater:** Strong fit — Dig Site has ledgey tile geometry; Hollow has platforms/districts. **Not implemented today** (jump only if `is_on_floor()` on press). High polish priority once movement feels settled; does not change fiction systems.
...
- Dig hits: pitch vary; Cap dig quieter / muffled vs Pit dig fuller / riskier
...
**Does not apply well:** Treating Harvest clock or district stock ticks as floating score spam.
...
4. **Dig feel as the loop’s “hit.”** Dust + pitched SFX + optional micro-squash; Cap quieter than Pit.
...
| Big shake on Cap digs | Undermines sneaking / taboo tension |
